 ...
 */
  ...
  for (q=0; q < NR_SCHED_QUEUES; q++) {
	...
	sums[q] = (i << NR_SCHED_QUEUES - 1 - q);
	sums[q] += (q > 0 ? sums[q - 1] : 0);
  }

  if (sums[NR_SCHED_QUEUES - 1] == 0)
	return NULL;

  ticket = random() % sums[NR_SCHED_QUEUES - 1];
  ...
  // vprintf("current prio: %d\n", q);

  ticket -= q > 0 ? sums[q - 1] : 0;
  i = (1 << NR_SCHED_QUEUES - 1 - q);
