...
void SwapQuery::execute_thd(Table::SizeType &counter) {
  ...
  for (auto &it = begin; it != end; ++it) {
    if (this->evalCondition(*it)) {
      std::swap((*it)[this->fieldId1], (*it)[this->fieldId2]);
      ++counter;
    }
  }
}
  ...
  try {
    ...
    if (result.second) {
      ...
      for (unsigned int i = 0; i < max_threads; ++i) {
        ...
        threads.emplace_back(&SwapQuery::execute_thd, this, ref(counters[i]));
      }
