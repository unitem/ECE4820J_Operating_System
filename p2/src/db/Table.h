...
public:
  ...
  public:
    ...
    std::vector<ValueType> getData() const { return it->datum; }

    void copyData(ObjectImpl &&target) { it->datum = target.it->datum; }
...
public:
  ...
  Iterator find(const std::string &key) {
    ...
    if (it != keyMap.end()) {
      return {data.begin() + static_cast<int>(it->second), this};
    }
    return {data.end(), this};
  }

  /**
   * Remove the last row of the table; keyMap is unchanged
   */
